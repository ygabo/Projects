...
class Classifier ( object ):
    ...
    def predict ( self , X ):
        ...
        Li0 = self.logpi[0]
        Li1 = self.logpi[1]
        Li1 += self.logtheta[1]*X.T + ( self.log1theta[1]*scipy.sparse.eye(X.shape[1],X.shape[0]) - self.log1theta[1]*X.T )  
        Li0 += self.logtheta[0]*X.T + ( self.log1theta[0]*scipy.sparse.eye(X.shape[1],X.shape[0]) - self.log1theta[0]*X.T ) 
