...
%%     <li> If <code>Cont</code> is an <code>cont_spec()</code>, then
...
%%   <ul>
%%     <li> If <code>What</code> is an <code>cont_spec()</code>, then
%%       it is interpreted as described for the
...
%   do_time(...), default_how_long(), default_return_spec()
...
%% it until it's total run-time is <code>default_how_long()</code>
%% seconds.
default_how_long() -> 0.8.
...
%%       possible.</li>
%%   </ul>
log(Log, Format, Data) -> [{now(), self(), [Format | Data]} | Log].
...
print_log(Log) ->
  case lists:flatten(Log) of
    ...
    FlatLog ->
      T0 = lists:foldl(  % find the earliest event time
	fun({T1, _P, _X}, T0) ->
	  D = timer:now_diff(T1, T0),
	  if
	    D <  0 -> T1;
	    D >= 0 -> T0
	  end
	end, element(1, hd(FlatLog)), FlatLog),

      % convert times to seconds and format the event description
      LogSec = [    { 1.0e-6*timer:now_diff(T, T0), P,
		      case X of
			[] -> "";
			[Fmt] -> Fmt;
