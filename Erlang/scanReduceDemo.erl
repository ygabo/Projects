-module(scanReduceDemo).
-export([distribute/3, distribute_pos/4, interest/3]).
...
-export([moving_tot/4, moving_tot/3, last_day/2, last_day/4]).
...
balance(W, Key, KeyB, Rate) ->
  wtree:scan(W,
    fun(ProcState) -> last_day(workers:get(ProcState, Key), Rate) end,
    fun(ProcState, L ) ->
      List =  workers:get(ProcState, Key),
      workers:put( ProcState, KeyB, lists:map(fun(X) -> penny_round(X) end, moving_tot(List, Rate, [], L))) end,
    fun([],R) -> R;
      (L,[]) -> L;
      (L,R) ->
      {Day,Money} = L,
      {A,B} = R,
      Total = Money * math:pow( 1+Rate , A - Day ) + B,
      {A, Total}
      end,
    []).
...
last_day([T], R, Total, CurrentDay) -> 
  {X, Y} = T,
  {X, Y + interest( Total, R, X - CurrentDay )};
last_day([H|T], R, Total, CurrentDay) ->
  {Day, Money} = H,
  last_day( T, R, interest( Total, R, Day - CurrentDay ) + Money, Day ).

  
moving_tot([T], Rate, Output) -> moving_tot([], Rate, Output, T );
moving_tot([H|T], Rate, Output) -> moving_tot(T, Rate, Output, H ).
moving_tot([], _, Output, _ ) -> Output;
moving_tot([T], Rate, Output, [] ) ->
  {_, T_money} = T,
  moving_tot( [], Rate, Output ++ [T_money], T );
moving_tot([H|T], Rate, Output, [] ) ->
  {_, H_money} = H,
  moving_tot( T, Rate, Output ++ [H_money], H);
moving_tot([T], Rate, Output, Latest ) ->
  { Day, Money } = Latest,
  { Curday, Curmoney } = T,
  MoneyCatchup = Money * math:pow( 1+Rate , Curday - Day ),
  moving_tot( [], Rate, Output ++ [MoneyCatchup+Curmoney], {Curday, MoneyCatchup+Curmoney} );
moving_tot([H|T], Rate, Output, Latest ) ->
  { Day, Money } = Latest,
  { Curday, Curmoney } = H,
  MoneyCatchup = Money * math:pow( 1+Rate , Curday - Day ),
  moving_tot( T, Rate, Output ++ [MoneyCatchup+Curmoney], {Curday, MoneyCatchup+Curmoney} ).
