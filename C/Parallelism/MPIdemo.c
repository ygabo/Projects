...
{
  int gap = M - arraylength;
  if ( arraylength < M )
    for ( int i = 0; i < arraylength; i ++ ) outarray[gap+i] = array[i];
  else
...
void rolling_average( double *src, double *dst, int count, int m,  MPI_Comm comm)
{
  ...
  // MPI initialize
  // these are actually not needed for this implementation :(
  MPI_Comm_rank(comm, &myID);
  MPI_Comm_size(comm, &numProcs);
  MPI_Op myOp;
  MPI_Datatype mType;
...
// create the B array given a list of transactions
// and an additional transaction ( this is what scan gives me)
// assume the additional transaction is the balance and the day
// of the last day of everything to my left
void construct_B_array( Transaction * myArray, int length, Transaction * bArray, Transaction f_l_global ){
  double lolbalance = 0;
  int lolday = 0, start = 0;
  
  // figure out how to start
  if( f_l_global.day == -1 ){
    lolbalance = myArray[0].balance;
    lolday = myArray[0].day;
    bArray[0].balance = ((double)((int)(round(lolbalance*100)))/100);
    bArray[0].day = lolday;
    start++;
  }
  else{
    lolbalance = f_l_global.balance;
    lolday = f_l_global.day;  
  }
  
  for (int i = start ; i < length; i++ ){
    lolbalance = previous_balance_to_current_day( lolbalance, myArray[i].day - lolday, RATE) + myArray[i].balance;
    lolday = myArray[i].day;
    bArray[i].balance = ((double)((int)(round(lolbalance*100)))/100);
    bArray[i].day = lolday;
  }
};
...
void balance( Transaction *src, double * dst, int count,  MPI_Comm comm)
{
  ...
  int stride=0, root_length=0;
  int last_node = 0;
  Transaction *myArray, *rootArray, *bArray;
  ...
  if(myID == RootProcess) {
    ...
    for(int i = 0; i < length_p; i++) { 
      ...
      if( stride+i == count-1 ) last_node = 0;
    }
    root_length = length_p;
    stride += length_p;
    for(int p=1; p < numProcs; p++)
    {
      ...
      for(int i = 0; i < length_p; i++)
      {
        ...
        if( stride+i == count-1 ) last_node = p;
      }
    ...
    stride = 0;
      ...
      bArray = (Transaction *)malloc(length_p*sizeof(Transaction));
  ...
  // send everyone last_node variable
  // so they know if they're the last node
  if( myID == RootProcess){
    length_p = root_length;
    myArray = (Transaction *)malloc(length_p*sizeof(Transaction));
    bArray = (Transaction *)malloc(length_p*sizeof(Transaction));
    ...
    for( int p =1; p < numProcs; p++ ) MPI_Send(&last_node, 1, MPI_INT, p, tag+3, comm);
  }
  else MPI_Recv(&last_node, 1, MPI_INT, RootProcess, tag+3, comm,&status);
  ...
  // SCAN
  if ( length_p > 0 ) MPI_Exscan(&f_l, &f_l_global, 1, ctype, myOp, comm);

  // construct B array (not sure if needed)
  // rounded to the nearest penny
  construct_B_array( myArray, length_p, bArray, f_l_global );
  
  // send the final balance to everyone
  if( last_node == myID ){
    // last element of last node of B array is the final balance
    lol = bArray[length_p-1].balance;
    for( int p=0; p< numProcs; p++)
      if( p != last_node ) 
       MPI_Send(&lol, 1, MPI_DOUBLE, p, tag+4, comm);
  }
  else
    MPI_Recv(&lol, 1, MPI_DOUBLE, last_node, tag+4, comm, &status);
...
int main(int argc, char **argv) {
  ...
  // every node knows final balance, i just picked root
  if( myID == RootProcess ) printf("Final balance: %f\n", total );
